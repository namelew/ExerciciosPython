...
class RespError(Exception):
    def __str__(self):
        return "Resposta inválida! O sistema aceita apenas Sim ou Não"
class Jogo(object):
    def __init__(self):
        self.__cartoes = ('''
        1   3   5   7   9   11  13  15
    ...
    def apresentação(self):
        ...
        print("Pense em um número e diga se eles está dentro desse cartão ou não")
    ...
    def recebeEntradaDoUsuário(self):
        ...
        while True:
            try:
                resp = input("O seu número está no cartão?Sim[S] ou Não[N]?").upper()[0].strip()
                if resp == 'S':
                    return True
                elif resp == 'N':
                    return False
                ...
                print("Reposta Inválida! Digite novamente!")
                continue
    ...
    def imprimeNumeroSecreto(self):
        ...
        """
        print("Processando...")
        print(f"O número que você pensou foi {self.__num}")
